...
9問（初級は8問）の質問に答えると、ユーザーに合ったテニスラケットを提案する診断ツール。提案したラケットに楽天アフィリエイトリンクを付けて収益化する。
...
src/
  app/
    ...
    page.tsx                 # 質問フォーム（9ステップ・初級は8ステップ、クライアントコンポーネント）
    result/
...
| パス | 画面 | レンダリング | 役割 |
|---|---|---|---|
| `/` | スタート画面＋質問フォーム | 導入・解説はサーバー、質問はクライアント | スタート画面（何ができるか・対応ブランド・ラケット選びの3つのポイント）を出し、「診断を始める」で9問へ。最終問で `/result?a=...` へ遷移 |
...
| # | 質問 | 選択 | 効き方 |
|---|---|---|---|
| Q1 | 性別 | 単一（3択・「回答しない」あり） | 重量レンジのずらし（理想値には効かせない） |
| Q2 | テニス歴・レベル | 単一（4択） | 理想値と重みの基準 |
| Q3 | プレースタイル | 単一（3択） | 理想値に加算、重みに乗算 |
| Q4 | スイングの大きさ | 単一（3択） | 理想値に加算、重みに乗算 |
| Q5 | いま困っていること | 最大2つ | 加算・乗算（2つ目は倍率 ×1.5 固定）＋ハードフィルタ |
| Q6 | 肘・肩の不安 | 単一（3択） | 加算・乗算＋ハードフィルタ（RA・厚み・重量下限） |
| Q7 | いまのラケットの重さ | 単一（5択） | ハードフィルタのみ（重量レンジ） |
| Q8 | 張る予定のストリング | 単一（3択） | 理想値に加算のみ |
...
このサイトのメイン利用者は初級者だが、Q4「スイングの大きさ」Q5「いま困っていること」Q7「いまのラケットの重さ」Q8「ストリング」は、テニスを始めたばかりの人には答えようがない。答えられない質問は当てずっぽうで選ばれるため、離脱を招くうえ診断の精度も下がる。

そこで Q2 で「初級」を選んだ場合だけ、同じ内部値のまま聞き方を差し替える（`src/components/Diagnosis.tsx`）。

| ステップ | 通常 | 初級 |
|---|---|---|
| Q4 | スイングの大きさ（コンパクト／標準／フルスイング） | 運動経験（あまりしてこなかった → `compact` ／ 人並みに動ける → `standard` ／ 他のスポーツ経験あり → `full`） |
| Q5 | いま困っていること（必須） | 不安に感じていること（言い回しを平易にし、**任意**＝飛ばせる） |
| Q7 | いまのラケットの重さ | 「いまラケットは持っていますか？」（「まだ持っていない・わからない」を先頭に置く） |
| Q8 | 張る予定のストリング | **聞かない**。もっとも一般的な `nylon` として扱う |

- 初級は1問減って**8問**になる。表示上の番号は `shown()` でずらし、`total` も 8 に切り替える。
- 内部の型・スコアリング・シェアURLの形式は一切変えていない。差し替えているのは質問文と選択肢のラベルだけなので、既存のシェアURLはそのまま動く。
...
type RacketSpec = {
  ...
  character: RacketCharacter;  // シリーズの性格（power/spin/control/comfort/allround）
  popularity: 'high' | 'standard'; // 定番度。high は各ブランドの主力ライン（23本）
...
- **データの誤りに注意**: 手入力のため実測と食い違うことがある。Pure Aero はバランスが 325mm（実測 320mm）、RA が 70（実測 69）になっており、そのせいで操作性とボレーが低く出て、提案にほぼ出てこなかった（修正後 2% → 9%）。Wilson Burn 100LS のストリングパターンも `16x19` だが実際は 18x16
...
質問は9問。8番目は廃止した予算の枠として予約したままにしてあり、性別は末尾（10番目）に足している（既存のシェアURLを壊さないため）。

```
a = Q1 - Q2 - Q3 - Q4 - Q5 - Q6 - Q7 - (予約) - Q9 - 性別
例: 1-0-1-1-0-2-1-3-0
```
...
| 10 | 性別 | 0=回答しない / 1=男性 / 2=女性。無い場合は「回答しない」として読む |
...
各スペックを**実用レンジの固定値（`SPEC_RANGES`）で 0〜100 に正規化**し、重み付き和で6軸を出す。
レンジ外は 0 / 100 に丸める。固定値にしているのは、機種を足してもほかの機種のスコアが動かないようにするため。

当初仕様書 3.3 の式から、妥当性のために2点を変えている。

- **コントロールから RA（フレームの硬さ）を外した**。以前は `control` と `comfort` の両方が「柔らかいほど高い」だったため、柔らかいラケットが2軸で得をし、硬いラケットは `power` の1軸でしか得をしなかった。その結果、RA55前後の機種が1位を占め、EZONE 100 や Pure Aero といった主力モデルが一度も提案されなかった。収まりを決めるのはフェイス面積・ストリングパターン・重量なので、そちらで組み立てている
- **快適性にフェイス面積を入れた**。以前は「柔らかい・薄い・重い」だけで決まり、97in² の上級者向けモデルが快適性トップクラスになっていた。スイートスポットが狭いとミスヒット時の衝撃が大きいので、フェイス面積を加点要素にしている
- **スピンを作り直した**。以前は `パターン + フェイスの大きさ + スイングウェイトの重さ` で、世界でもっともスピンがかからない部類の Pro Staff 97 v14 が上位に、各社のスピン専用モデル（VCORE / Pure Aero / Extreme）はパワーモデルより下に来ていた。フェイス面積とスイングウェイトは加点ではなく、最適値（101in² / SW305）から離れたぶんを引く**減点項**にしている。加点にすると標準的な 16x19・100in² がすべて高得点になってしまうため
- **シリーズの性格（`character`）を入力に加えた**。VCORE 100 と EZONE 100 はスペックがほぼ同一（300g・100in²・16x19、違いは SW310/300 と RA70/68）で、**式だけでは区別できない**。実際の差はフレーム形状・グロメット・素材にあり、それはメーカーがシリーズに与えた性格として表れる。パワー / スピン / コントロール / 快適性 / オールラウンドの5分類を持たせ、パワー・コントロール・スピン・快適性の各軸で `CHARACTER_WEIGHT`（0.30〜0.55）ぶん効かせる。残りはスペックから算出するので、同じシリーズでも重量やフェイスが違えばスコアは変わる

**`character` の分類根拠**（メーカーと各社シリーズ解説）: Yonex は EZONE=パワー / VCORE=スピン / PERCEPT=コントロール、Babolat は Pure Drive=パワー / Pure Aero=スピン / Pure Strike=コントロール、Dunlop は FX=パワー / SX=スピン / CX=コントロール、Head は Extreme=スピン / Gravity・Prestige=コントロール / Speed・Radical=オールラウンド、Wilson は Ultra=パワー / Blade・Pro Staff=コントロール / Burn=スピン / Clash・Triad=快適性。
...
4. フィルタを積む: Q1「初級・初中級」→ フェイス 98in² 以上（95〜97in² のプレイヤーズラケットを出さない）/ Q4「回転がかからない」→ 18x20 を除外 / Q4「振り遅れる」→ 重量上限 / Q5 → RA・厚み・重量下限（痛みがある場合 285g 以上は**意図的な仕様**）/ Q6 → 重量レンジ / Q9 → ブランド

**性別の扱い**: 重量レンジを軽い側へずらすだけ（女性 -10g）。理想値（パワー・コントロールなど）には効かせない。「女性だから飛ぶラケット」といった決めつけは診断の精度を下げるため。現在の重さを答えた人にはほとんど影響せず、「わからない」を選んだ人の推定精度が上がる。

**理想値の置き方**: 快適性・操作性・ボレーは「足りない分だけ」減点する片側ペナルティなので、理想値は「目標」ではなく**最低ライン**として置く。分布の中央値より大きく上に置くと、超過が減点されないぶん「柔らかく軽いほど得」の並べ替えになり、特定の機種が1位を占めてしまう（実際に起きていた）。

### 6.2.1 定番度（`popularity`）

適合度に `POPULARITY_BONUS`（5点）を加点する。各ブランドの主力ライン23本が対象。

理由: スペック上は中庸でも、店頭・通販で定番として置かれているモデル（EZONE 100 / Pure Drive /
VCORE 100 など）は、知名度・入手しやすさ・レビューの多さで実際の選択肢になりやすい。
加点前はこれらが3本に入る割合が1〜3%しかなく、「自分のラケットが出てこない」状態だった。

**5点が上限**。8点・12点も試したが、妥当性テスト（回答に合った機種が出るか）が落ちるため、
スペックの差を覆さない範囲に留めている。

### 6.2.2 ブランドの分散

`diversify` で2位・3位を選ぶとき、`BRAND_DIVERSITY_TOLERANCE`（4点）以内の差なら、
まだ出していないブランドのモデルを優先する。別シリーズ・役割の条件はそのまま。

導入前は3本中2本以上が同じブランドになるケースが46%（3本とも同一が7%）あり、
Wilson が提案枠の37%を占めていた。導入後は 14%（3本とも同一は2%）。
