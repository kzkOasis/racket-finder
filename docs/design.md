...
type RacketSpec = {
  ...
  spinDesign?: boolean;  // メーカーがスピン向けとして設計しているモデル（スピン軸で加点）
...
- **スピンを作り直した**。以前は `パターン + フェイスの大きさ + スイングウェイトの重さ` で、世界でもっともスピンがかからない部類の Pro Staff 97 v14 が上位に、各社のスピン専用モデル（VCORE / Pure Aero / Extreme）はパワーモデルより下に来ていた。フェイス面積とスイングウェイトは「大きい・重いほど良い」ではなく**101in² / SW305 を頂点とする山型**にし、スペックに現れないスピン設計（空力フレーム・グロメット）を `spinDesign` として持たせた。VCORE 100 と EZONE 100 はスペックがほぼ同一（SW310/300、RA70/68）で、式だけでは区別できないため
